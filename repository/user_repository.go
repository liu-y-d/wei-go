...
type IUserRepository interface {
	...
	WxLogin(user *vo.WxLoginRequest) (*model.User, error) // 登录
...
func (ur UserRepository) WxLogin(wxRequest *vo.WxLoginRequest) (*model.User, error) {
	// jscode2Session
	url := fmt.Sprintf("https://api.weixin.qq.com/sns/jscode2session?appid=%s&secret=%s&js_code=%s&grant_type=authorization_code", config.Conf.Wechat.Appid, config.Conf.Wechat.Secret, wxRequest.Code)
	// 发起请求

	res, _ := http.Get(url)

	//var wxMap map[string]string
	//json.NewDecoder(res.Body).Decode(&wxMap)
